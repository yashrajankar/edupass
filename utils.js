...
window.handleFetchError = function handleFetchError(error, operation) {
    ...
    if (window.location.protocol === 'file:') {
        showToast('Please access this page through a web server (http://localhost:3002) rather than opening the file directly.', 'error', 8000);
        return;
    }
    
    // Check if we're accessing through a different port than the API server
    const currentPort = window.location.port;
    if (currentPort && currentPort !== '3002' && (window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1')) {
        showToast(`Please access this page through http://localhost:3002 rather than port ${currentPort}`, 'error', 8000);
        return;
    }
