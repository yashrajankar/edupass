    ...
    try {
        ...
        seatingPlans.forEach((plan, index) => {
            ...
            if (plan.planData && plan.planData.roomId) {
                // Original structure with roomId in planData
            ...
            // NEW: Extract room ID from seat IDs if not found in planData
            if (!roomId && seats && seats.length > 0 && seats[0].id) {
                // Extract room ID from the first seat ID (format: "seat-ROOMID-INDEX")
                const seatIdParts = seats[0].id.split('-');
                if (seatIdParts.length >= 2) {
                    roomId = parseInt(seatIdParts[1]);
                }
            }
            ...
            if (roomId && roomMap[roomId]) {
                ...
                if (Array.isArray(seats)) {
                    seats.forEach((seat, seatIndex) => {
                        ...
                        if (seat && !seat.isTeacherDesk && seat.rollNo) {
                        ...
                        } else if (seat && !seat.isTeacherDesk && seat.studentId) {
